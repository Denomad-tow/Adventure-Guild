-- 월드 보스 도전 난이도(쉬움/보통/어려움) 선택 + 등급별 전리품 품질 차등 + 보스 전용 세트.
-- - 새로 만든 캐릭터는 레벨이 낮으면 "보통/어려움"을 고를 수 없다 (서버에서도 다시 확인해서 막는다).
-- - 어려운 난이도로 도전할수록 전리품 등급(영웅/전설/신화)이 더 잘 나오고, 옵션 개수도 더 많다.
-- - 월드 보스 전리품은 항상 "일곱 군주의 유산" 전용 세트로 나온다 (필드 세트보다 세트 효과가 세다).
-- Supabase 대시보드 > SQL Editor 에서 이 파일 내용을 그대로 실행하세요.

alter table public.world_boss_challenges add column if not exists tier text not null default 'easy';

-- 예전 2개 인자 버전은 더 이상 안 쓰이므로 지운다 (그대로 두면 인자 개수가 달라 별개 함수로 남아 헷갈린다).
  ...
  if v_was_lethal then
    ...
    for v_participant in
      select c.user_id, sum(c.damage) as total_damage,
        max(case c.tier when 'hard' then 2 when 'normal' then 1 else 0 end) as tier_rank
      from public.world_boss_challenges c
      where c.lord_index = v_lord_index
      group by c.user_id
    loop
      v_participant_reward := v_participation_reward
        + round(v_contribution_pool * v_participant.total_damage::float / v_max_hp);

      v_tier_rank := v_participant.tier_rank;
      v_tier_multiplier := case v_tier_rank when 2 then 1.3 when 1 then 1.15 else 1.0 end;
      ...
      if v_loot_grade is not null then
        ...
        for i in 1..v_option_count loop
          ...
          v_loot_options := v_loot_options || jsonb_build_array(
            jsonb_build_object(
              'type', v_type,
              'value', round((v_min + random() * (v_max - v_min)) * v_grade_multiplier * v_tier_multiplier)::int
            )
          );
        end loop;
