-- 장비 부위가 6개 → 10개로 바뀌면서(014번 참고), 월드 보스 처치 보너스 드랍이 여전히 옛날 부위
-- 목록(그 중에는 이제 없어진 'accessory'도 있었음)을 사용하고 있어서 그대로 두면 장비 저장에 실패한다.
-- 새 부위 목록으로 맞춰준다. (014_equipment_slots_and_types.sql을 먼저 실행하세요)
